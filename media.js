...
function calculadoraMedia(){
  ...
  const ultimaMensagem = divMedias.querySelector('.ultimaMensagem');
  if (ultimaMensagem) {
      divMedias.removeChild(ultimaMensagem);
  }

  const situacaoAluna = document.createElement('p');
  situacaoAluna.classList.add('ultimaMensagem'); 
  divMedias.appendChild(situacaoAluna);
  situacaoAluna.innerHTML = mensagem;
